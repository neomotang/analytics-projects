...
- Cross-filtering and highlighting is enabled between visuals. The user can also: enter a `Top N` parameter for the top stations information to be summarized; choose a `swap parameter` (either number of trips or trip length) to plot over time; and `show/hide` a map visual displaying the location of the selected top N stations.
